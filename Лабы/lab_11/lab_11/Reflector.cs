...
namespace lab_11
{
    public static class Reflector
    {
       ...
       // определение имени сборки, в которой определён класс
...
// получает информацию о полях и свойствах класса (возвращает IEnumerable<string>)
      ...
      // получает все реализованные классом интерфейсы (возвращает IEnumerable<string>);
       ...
       // метод Invoke, который вызывает метод класса, при этом значения 
        ...
        //---
        private static readonly string fileForInformationPath = @"F:\.Уник\2 курс\ООП С#\Лабы\lab_11\Reflection.txt";
        private static readonly string fileForInvokePath = @"F:\.Уник\2 курс\ООП С#\Лабы\lab_11\Invoke.txt";
        ...
        public static object Create( int width, int height )
        {
            ...
            object obj = info.Invoke(new object[] { title, price });
            return obj;
        }
        */
    }
