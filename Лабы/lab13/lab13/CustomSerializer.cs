...
namespace lab13
{
    public enum SERIALIZETYPE
    {
     ...
     {
        public static void Serialize( List<T> list, SERIALIZETYPE type, string path ) //metod
        {
            switch (type)
            {
                ...
                case SERIALIZETYPE.JSON:
                    {
                        DataContractJsonSerializer js = new DataContractJsonSerializer(typeof(List<T>));//сериализует контракт в формате JSON. 
                        using (FileStream fileStream = new FileStream(path + $@"\{list.GetType()}.json", FileMode.OpenOrCreate))
                        {
                            js.WriteObject(fileStream, list);// куда и что
                        }
        ...
        public static T Deserialize( string path )
        {
            ...
            switch (format)
            {
                ...
                case "json":
                    {
                        DataContractJsonSerializer js = new DataContractJsonSerializer(typeof(Island));
                        using (FileStream fileStream = new FileStream(path, FileMode.Open))
                        {
